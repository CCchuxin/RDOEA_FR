function functionvalue = MSE(populationlum,y_sub_image,cb_sub_image,cr_sub_image, y_sub_dct, cb_sub_dct,cr_sub_dct,repeat_height, repeat_width, outputName, fast_mode, mse_table, rate_table)
      lum_table  = populationlum(:,1:64);
      chrom_table = populationlum(:,65:128);
      popnum = size(populationlum,1);
     for a=1:popnum
        initGlobals(lum_table(a,:),chrom_table(a,:));
        lum_quant = populationlum(a,1:64);
        chrom_quant = populationlum(a,65:128);
        if fast_mode == 1
            ret(a,:) = jpeg_encoder_func(y_sub_image,cb_sub_image,cr_sub_image,y_sub_dct,cb_sub_dct,cr_sub_dct, repeat_height, repeat_width, outputName, lum_quant,chrom_quant, fast_mode,mse_table, rate_table);   
        else
            ret(a,:) = jpeg_encoder_func(y_sub_image,cb_sub_image,cr_sub_image, y_sub_dct, cb_sub_dct,cr_sub_dct,repeat_height, repeat_width, outputName, lum_quant, chrom_quant,fast_mode, mse_table, rate_table);
        end
     end      
...
function initGlobals(lum_table,chrom_table)
...
%              72 92 95 98 112 100 103 99];
    ...
    global quantLumMatrix;
     quantLumMatrix = reshape( lum_table, 8, 8)';%亮度量化表 wqj

    
    global divLumMatrix;
    divLumMatrix = divMat(lum_table);

    global quantChromMatrix;
    quantChromMatrix = reshape(chrom_table, 8, 8)'; %wqj
    
    global divChromMatrix;
    divChromMatrix = divMat(chrom_table);
